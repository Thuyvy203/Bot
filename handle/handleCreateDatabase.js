    ...
    return async function ({ event }) {
        ...
        try {
            if (!allThreadID.includes(threadID) && event.isGroup == !![]) {
                ...
                let threadName = threadIn4.threadName
                const setting = {};
                ...
                threadInfo.set(threadID, dataThread, threadName);
                const setting2 = {};
                ...
                for (singleData of threadIn4.userInfo) {
                    ...
                    try {
                        ...
                        logger(global.getText('handleCreateDatabase', 'newUser', (`Người dùng mới:  `) + (`${singleData.name}`) + "  ->  " + (`${singleData.id}`)), '[ NGƯỜI DÙNG MỚI ]'));
                    } catch(e) { console.log(e) };
                }
                logger(global.getText('handleCreateDatabase', 'newThread',(`Nhóm mới: `) + (`${threadID}`) + "  ->  " + (`${threadIn4.threadName}`)), '[ NHÓM MỚI ]');
            }
            if (!allUserID.includes(senderID) || !userName.has(senderID)) {
                ...
                logger(global.getText('handleCreateDatabase', 'newUser', (`Người dùng mới: `) + (`${singleData.name}`) + " -> " + (`${senderID}`)), '[ NGƯỜI DÙNG MỚI ]');
            }
