    ...
    case "unbanthread":
      {
        ...
        api.sendMessage(`Thông báo từ ${name}«\n\n-Nhóm ${strS} của bạn dã được gỡ ban \n\n-Có thể sử dụng BOT ngay bây giờ`, uidS, () =>
    ...
    case "-t":
      {
        ...
        return api.sendMessage(listbanViews.length != 0 ? api.sendMessage(`🎀 Hiện tại có ${listbanViews.length} nhóm bị ban\n\n${listbanViews.join("\n\n")}` +
          "\n\nReply tin nhắn này + số thứ tự, có thể rep nhiều số, cách nhau bằng dấu cách",
          threadID, (error, info) => {
          ...
          messageID
        ) : "Hiện tại kkhoong có nhóm nào bị ban 😻", threadID, messageID);
      }
    case "user":
    case "u":
    case "-u":
      {
        ...
        return api.sendMessage(listbanViews.length != 0 ? api.sendMessage(`🌸 Hiện tại có ${listbanViews.length} người dụng bị ban\n\n${listbanViews.join("\n\n")}` +
          "\n\nReply tin nhắn này + 𝐬số thứ tự, có thể rep nhiều số, cách nhau bằng dấu cách",
          threadID, (error, info) => {
