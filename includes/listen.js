...
𑁍 Lê Nguyễn Thúy Vy 𖤐 Đẹp gái Đáng yêu 
...
`, "Bơ");
  logger(`${global.config.ADMINBOT} - Bơ is main!!!`, "ADMINBOT INFO");
  logger(`${global.config.NDH}`, "NDH INFO");
  logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] ${(!global.config.BOTNAME) ? "This bot was made by Bơ" : global.config.BOTNAME}`, "BOT INFORMATION");
  ...
  return async (event) => {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID) &&!ndhBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}duyetbox`) {
        adminBot.forEach(e => {
          api.sendMessage(`=== 『 DUYỆT BOX 』 ===\n━━━━━━━━━━━━━━━━━━\n➜ Tên: ${threadName}\n➜ ID: ${event.threadID}\n➜ Đã yêu cầu được duyệt\n➜ Liên hệ: https://www.facebook.com/BerXinh.203`, e);
        })
        return api.sendMessage({
    body: `=== 『 REQUEST SUCCESS 』 ===\n━━━━━━━━━━━━━━━━━━\n➜ Đã gửi yêu cầu đến ${admin.length} ADMINBOT`, attachment: (await axios.get((await axios.get(`http://toan-nguyen.toan2005aye.repl.co/images/anime`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID, () => {
...
[🏘] Nhóm: ${threadName}
[🧸] TID: ${event.threadID}
[🌟] Nhóm bạn chưa được duyệt, dùng: ${prefix}duyet 💞\n━━━━━━━━━━━━━━━━━━\n
`, attachment: (await axios.get((await axios.get(`http://toan-nguyen.toan2005aye.repl.co/images/anime`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID);
    ...
    switch (event.type) {
      //<--Thay đổi ảnh nhóm-->//
          ...
          return api.sendMessage({
    body: `${msg}`, attachment: (await axios.get((await axios.get(`http://toan-nguyen.toan2005aye.repl.co/images/anime`)).data.url, {
                    responseType: 'stream'
                })).data
}, event.threadID);
        }
