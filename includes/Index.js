...
try {
    ...
    if (!global.Fca.Require.fs.existsSync(process.cwd() + '/FcaSetting.json')) {
        global.Fca.Require.fs.writeFileSync(process.cwd() + "/FcaSetting.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));
        process.exit(1);
    }

try {
    var Data_Setting = require(process.cwd() + "/FcaSetting.json");
}
catch (e) {
    global.Fca.Require.logger.Error('Detect Your FcaSetting Settings Invalid!, Carry out default restoration');
    global.Fca.Require.fs.writeFileSync(process.cwd() + "/FcaSetting.json", JSON.stringify(global.Fca.Data.ObjFastConfig, null, "\t"));     
    process.exit(1)
}
    if (global.Fca.Require.fs.existsSync(process.cwd() + '/FcaSetting.json')) {
        ...
        for (let i of All_Variable) {
            if (Data_Setting[i] == undefined) {
                ...
                global.Fca.Require.fs.writeFileSync(process.cwd() + "/FcaSetting.json", JSON.stringify(Data_Setting, null, "\t"));
            }
            else continue; 
        } //Check Variable
