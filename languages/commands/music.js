...
module.exports.handleReply = async function ({
    ...
    try {
        ...
        if (fs.statSync(__dirname + `/cache/${event.senderID}.mp3`).size > 104000000) return api.sendMessage('Không thể gửi video vì dung lượng lớn hơn 25mb', event.threadID, () => unlinkSync(__dirname + `/cache/${event.senderID}.mp3`), event.messageID);
...
module.exports.run = async function ({
    ...
    if (args.length == 0 || !args) return api.sendMessage('» Phần tìm kiếm không thể bỏ trống', event.threadID, event.messageID);
    ...
    if (args.join(" ").indexOf("https://") == 0) {
        ...
        try {
            ...
            var body = `»🔎 Có ${link.length} danh sách trùng với từ khoá tìm kiếm của bạn:\n\n${msg}\n» Hãy reply theo STT để chọn 1 trong các tìm kiếm trên`
